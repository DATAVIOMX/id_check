...
class TextCheck(Resource):
    ...
    def post(self):
        ...
        parser.add_argument("api_key", type=str, required=True)
        parser.add_argument("emision", None)
        parser.add_argument("ocr_vertical", None)
        parser.add_argument("ocr_horizontal", None)
        parser.add_argument("cic", None)
        parser.add_argument("cve_elec", None)
        args = parser.parse_args()
        print(args)
        conn = db.connect("dbname='id_check_db' user='otto' host='localhost' password=ottoman")
        cur = conn.cursor()
        ...
        if not db_response:
            # log call
            # get max callid
            cur.execute("SELECT max(callid) from api_calls")
            maxid = cur.fetchone()[0] + 1
            call_dt = datetime.datetime.now().strftime("%Y-%b-%d (%H:%M:%S.%f)") # corregir esto y pasar a str
            print(call_dt, type(call_dt))
            print(type(json.dumps(req_data)))
            print(json.dumps(req_data))
            cur.execute("""INSERT INTO api_calls (callid, call_date, call_point,
                        status_code, call_text, response) VALUES 
                        (%s,%s,%s,%s,pgp_sym_encrypt(%s, 'longsecretencryptionkey'),%s)""",
                        (maxid, call_dt, 'id-check', 404, json.dumps(req_data),'{"error":"key does not exist"}',))
            conn.commit()
            conn.close()
            return {"error": "key does not exist"}, 404
        ...
        if status == 1:
            # log call
            cur.execute("""INSERT INTO api_calls (callid, userid, call_date, call_point,
                    status_code, call_text, response) VALUES 
                    (%s,%s,%s,%s,%s,pgp_sym_encrypt(%s,'longsecretencryptionkey'),
                    pgp_sym_encrypt(%s,'longsecretencryptionkey'))""",
                        (maxid,userid, call_dt, 'id-check', 403, json.dumps(req_data),
                         '{"error":"payment processing in progress"}',))
            conn.commit()
            conn.close()
            return {"error": "payment processing in progress"}, 401
        if status == 2:
            ...
            cur.execute("""INSERT INTO api_calls (callid,userid, call_date, call_point,
                    ...
                    pgp_sym_encrypt(%s,'longsecretencryptionkey'),
                    pgp_sym_encrypt(%s,'longsecretencryptionkey'))""",
                        (maxid, userid, call_dt, 'id-check', 402,
        ...
        results = id_check_api.check_id_text(args)
        # log call
        cur.execute("""INSERT INTO api_calls (callid, userid, call_date, call_point,
                status_code, call_text, response) VALUES 
                (%s,%s,%s,%s,%s,pgp_sym_encrypt(%s,'longsecretencryptionkey'),
                pgp_sym_encrypt(%s,'longsecretencryptionkey'))""",
                    (maxid,userid, call_dt, 'id-check', 200, json.dumps(req_data),
                     json.dumps(results),))
        conn.commit()
        conn.close()
